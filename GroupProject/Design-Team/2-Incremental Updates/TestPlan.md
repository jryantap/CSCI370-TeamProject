...
| TC13 | Successfully check the cost of an item. | 1. Have an item. | Check database for cost of the item. | ItemName="blah" | Positive float number |
| TC14 | Error message on an invalid item number(count). | Have an Item | Open app and check the number of Item in the list | ItemName="blah" <br> ItemNumber = a | Item Number must be zero or more integer |
