...
| :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- | :-- | :--
| TC01 | Successful login to app. | 1. A valid user account to login. | 1. Open App. <br> 2. Enter UserID. <br> 3. Enter password. 4. Click "Login". | UserID = tjesus, Password = 370Spring18 | User is logged in successfully. | N/A | N/A | Thom De Jesus | N/A | N/A | N/A | Android Studio | Test Case not valid. Login screen not yet implemented.
| TC02 | Error message on unsuccessful login. | 1. A valid user name to login. | 1. Open app. <br> 2. Enter UserID. <br> 3. Enter password. <br> 4. Click "Login". | UserID = tjesus, Password = 30Spring18 | Error message is displayed and user is not logged in. | N/A | N/A | Thom De Jesus | N/A | N/A | N/A | Android Studio | Test Case not valid. Login screen not yet implemented.
| TC03 | Successful creation of new list. | None | 1. Open app. <br> 2. Click "+" button on top of screen. <br> 3. Enter the name of the list. <br> 4. Click "SUBMIT" when done. | List name = weekly | User successfully creates a new list named "weekly". | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
| TC04 | Error message on attempting to create a new list with a list name already in use. | 1. App is open. <br> 2. Lists are already created. | 1. Click "+" button on top of screen. <br> 2. Enter the name of the list. | List name = weekly | Error message is displayed and the user has not created a new list. | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
| TC05 | Successful addition of an item to a list from the item database. | 1. User created a list. <br> 2. User selected a list. <br> 3. Item exists in the database. <br> 4. Item being added is not on the selected list. | 1. User searches for the item name or item type in the space provided. <br> 2. User selects the item from the item database. <br> 3. User double clicks the item name. <br> 4. User enters quantity and clicks "OK". | List name = weekly, Item name = apple, quantity = 2 | Item "apple" with quanitity "2" is successfully added to the list named "weekly". | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
| TC06 | Error message on attempting to add an item already on a list from the item database. | 1. User created a list. <br> 2. User selected a list. <br> 3. User added items to the list. | 1. User searches for the item name or item type in the space provided. <br> 2. User selects the item from the item database. <br> 3. User clicks "Add Item" or double clicks the item name. | List name = weekly, Item name = apple | Error message is displayed and item milk is not added. | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | Error message appears as: "Item may alread in list." (Spelling error). Fix: Result.java line 100: "Item may already be in list."
| TC07 | Successful deletion of an item from a list. | 1. User has created a list. <br> 2. User has items in a list. | 1. User selects a list to delete an item from. <br> 2. User selects an item in the list to delete by clicking the check box to the right of the item. <br> 3. User clicks on the trash icon at the top of the screen. <br> 4. User confirms deletiong. | List name = weekly, Item name = apple | Item "apple" is successfully deleted from list "weekly".| As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
| TC08 | Error message on attempting to delete an item from a list without selecting an item. | 1. User has created a list. <br> 2. User has items on a list. | 1. User selects a list to delete an item from. <br> 2. User clicks on the trash icon at the top of the screen. | Error message is displayed and no deletion occurs. | No error message is displayed. | Fail | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
| TC09 | Successful update to an item's quantity. | 1. User has created a list. <br> 2. User has items on a list. | 1. User selects a list. <br> 2. User selects an item in the list. <br> 3. User enters quantity amount in the space provided and clicks "OK". | List name = weekly, item name = apple, quantity = 3 | Item "apple" has its quantity updated from "2" to "3" in list "weekly". | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
...
| TC13 | Successfully clear one check-off mark from a list. | 1. User has created a list. <br> 2. User has items on a list. <br> 3. Box next to item has a check-off mark. | 1. User selects a list. <br> 2. User clicks on a box with a check-off mark. | List name = weekly, item name = apple, box = checked-off | Check-off mark is cleared next to item "apple" and the box is now empty. | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
| TC14 | Successfully clear all check-off marks from a list at once. | 1. User has created a list. <br> 2. User has items on a list. <br> 3. One or more items are checked-off. | 1. User selects a list. <br> 2. User clicks on the 3 vertical dots and selects "clearCheck". | List name = weekly | All check-off marks have been cleared and items still remain in the list. | As expected. | Pass | Thom De Jesus | N/A | Joseph Huaynate | 4/20/18 | Android Studio | None
